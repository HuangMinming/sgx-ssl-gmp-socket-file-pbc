                                   ...
                                   unsigned char *ptr_rk_A_B, size_t ptr_rk_A_B_len)
{
    ...
    int a1_len = element_from_bytes(a1, ptr_a1);
    int g_b2_len = element_from_bytes(g_b2, ptr_g_b2);
    if (a1_len != ptr_a1_len ||
        g_b2_len != ptr_g_b2_len)
    {
        sgx_printf("a1_len = %d, ptr_a1_len = %d\n", a1_len, ptr_a1_len);
        sgx_printf("g_b2_len = %d, ptr_g_b2_len = %d\n", g_b2_len, ptr_g_b2_len);
        sgx_printf("exit \n");
        element_clear(a1);
        element_clear(g_b2);
    ...
    sgx_printf("\n rk_A_B_data = ");
    element_to_bytes(rk_A_B_data, rk_A_B);
                    ...
                    unsigned char *ptr_Z_b2_a1_k, size_t ptr_Z_b2_a1_k_len)
{
    ...
    int g_k_len = element_from_bytes(g_k, ptr_g_k);
    int rk_A_B_len = element_from_bytes(rk_A_B, ptr_rk_A_B);
    int m_Z_a1_k_len = element_from_bytes(m_Z_a1_k, ptr_m_Z_a1_k);

    if (g_k_len != ptr_g_k_len ||
        rk_A_B_len != ptr_rk_A_B_len ||
        m_Z_a1_k_len != ptr_m_Z_a1_k_len)
    {
        sgx_printf("g_k_len = %d, ptr_g_k_len = %d\n", g_k_len, ptr_g_k_len);
        sgx_printf("rk_A_B_len = %d, ptr_rk_A_B_len = %d\n", rk_A_B_len, ptr_rk_A_B_len);
        sgx_printf("m_Z_a1_k_len = %d, ptr_m_Z_a1_k_len = %d\n", m_Z_a1_k_len, ptr_m_Z_a1_k_len);
        sgx_printf("exit \n");
        element_clear(g_k);
        element_clear(rk_A_B);
        element_clear(m_Z_a1_k);
    ...
    unsigned char *ptr_b2, size_t ptr_b2_len)
{
    ...
    int m_Z_a1_k_len = element_from_bytes(m_Z_a1_k, ptr_m_Z_a1_k);
    int Z_b2_a1_k_len = element_from_bytes(Z_b2_a1_k, ptr_Z_b2_a1_k);
    int b2_len = element_from_bytes(b2, ptr_b2);

    if (m_Z_a1_k_len != ptr_m_Z_a1_k_len ||
        Z_b2_a1_k_len != ptr_Z_b2_a1_k_len ||
        b2_len != ptr_b2_len)
    {
        sgx_printf("m_Z_a1_k_len = %d, ptr_m_Z_a1_klen = %d\n", m_Z_a1_k_len, ptr_m_Z_a1_k_len);
        sgx_printf("Z_b2_a1_k_len = %d, ptr_rk_A_B_len = %d\n", Z_b2_a1_k_len, ptr_Z_b2_a1_k_len);
        sgx_printf("b2_len = %d, ptr_b2_len = %d\n", b2_len, ptr_b2_len);
        sgx_printf("exit \n");
        element_clear(m_Z_a1_k);
        element_clear(Z_b2_a1_k);
        element_clear(b2);
    ...
    element_t *ptr_a2)
{
    ...
    if (element_cmp(*ptr_m, beta_alpha_a1_invert) == 0)
    {
        sgx_printf("ptr_m == beta_alpha_a1_invert.\n");
    }
    else
    {
        sgx_printf("ptr_m != beta_alpha_a1_invert.\n");
    ...
    if (element_cmp(*ptr_m, beta_alpha_a2_invert) == 0)
    {
        sgx_printf("ptr_m == beta_alpha_a2_invert.\n");
    }
    else
    {
        sgx_printf("ptr_m != beta_alpha_a2_invert.\n");
    ...
    element_t *ptr_a1)
{
    ...
    if (element_cmp(*ptr_m, beta_pair_alpha_g_a1) == 0)
    {
        sgx_printf("ptr_m == beta_pair_alpha_g_a1.\n");
    }
    else
    {
        sgx_printf("ptr_m != beta_pair_alpha_g_a1.\n");
    ...
    element_t *ptr_b2)
{
    ...
    if (element_cmp(*ptr_m, beta_alpha_b2_invert) == 0)
    {
        sgx_printf("ptr_m == beta_alpha_b2_invert.\n");
    }
    else
    {
        sgx_printf("ptr_m != beta_alpha_b2_invert.\n");
...
void t_sgxpbc_pairing_test()
{
    ...
    First_Level_Decryption(&m, &Z_a1_k, &m_Z_k, &Z_a2_k, &a1, &a2);

    Second_Level_Decryption(&m, &g_k, &m_Z_a1_k, &a1);

    B_Decryption(&m, &m_Z_a1_k, &Z_b2_a1_k, &b2);
    element_clear(a1);
    element_clear(a2);
    element_clear(Z_a1);
    element_clear(g_a2);
