以Intel SGX SampleEnclave 项目为基础，增加sgxssl，可以在enclave内调用sgxssl所支持的函数，支持C/C++。增加sgx-gmp，可以在enclave内调用sgx-gmp所支持的函数，仅支持C，不支持C++，不支持I/O相关操作。
在app中增加socket的支持，使用select，可以对外提供socket接入服务。
App中增加对文件下载的支持，在本地读取文件后，调用ecall做处理，处理后的数据，写入输出流，等待客户端全部接收完毕后关闭输出流。
------------------------
